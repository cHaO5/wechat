...
class WeChatEmotionScollView: UICollectionView {
    ...
    fileprivate weak var currentMagnifierCell: WeChatEmotionCell?
    ...
    func initialize() {
        self.magnifierImageView = UIImageView(image: WeAsset.Emoticon_keyboard_magnifier.image)
    ...
    func endBackspaceTimer() {
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(WeChatEmotionScollView.startBackspaceTimer), object: nil)
    ...
    /**
     ...
     - returns: 表情 Cell
     */
    func cellForTouches(_ touches: Set<UITouch>) -> WeChatEmotionCell? {
        ...
        let cell: WeChatEmotionCell = self.cellForItem(at: newIndexPath) as! WeChatEmotionCell
        return cell
    }
     ...
     */
    func showMagnifierForCell(_ cell: WeChatEmotionCell) {
...
// MARK: - @extension WeChatEmotionScollView
extension WeChatEmotionScollView {
    /**
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if cell.isDelete {
            self.endBackspaceTimer()
            self.perform(#selector(WeChatEmotionScollView.startBackspaceTimer), with: nil, afterDelay: 0.5)
        }
    }
