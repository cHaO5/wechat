//
//  WeMessageActionFloatView.swift
//  WeWeChat
//
...
class WeMessageActionFloatView: UIView {
    ...
    fileprivate func initContent() {
        ...
        let actionImages = [
            WeAsset.Contacts_add_newmessage.image,
            WeAsset.Barbuttonicon_add_cube.image,
            WeAsset.Contacts_add_scan.image,
            WeAsset.Receipt_payment_icon.image,
        ]
        ...
        let bubbleMaskImage = WeAsset.MessageRightTopBg.image.resizableImage(withCapInsets: stretchInsets, resizingMode: .stretch)
        ...
        for index in 0 ..< actionImages.count {
            ...
            itemButton.addTarget(self, action: #selector(WeMessageActionFloatView.buttonTaped(_:)), for: UIControlEvents.touchUpInside)
...
//        tap.rx.event.bind(onNext: {
//            _ in self.hide(true)
//        }).disposed(by: disposeBag)
//        tap.rx.event.subscribe(onNext: {
//            [weak self] _ in self.hide(true)
//        }).disposed(by: disposeBag)
...
/**
 *  WeMessageViewController Float view delegate methods
 */
